...
키 검색 탭의 [저장], JSON 추가 탭의 [반영]이 성공하면 **현재 페이지**에서 해당 key에 연결된 TEXT 노드를 찾아 값을 교체하고, 반영된 개수를 알려줍니다.

| 조건 | 동작 |
|---|---|
| 노드 찾는 방법 | 노드의 `pluginData`에 저장된 key로 역추적합니다. 즉 **한 번이라도 동기화되어 key가 연결된 노드만** 대상입니다 |
| 범위 | 현재 페이지만 탐색합니다. 다른 페이지의 노드는 반영되지 않습니다 |
